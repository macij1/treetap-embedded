...
void initBLE() {
    ...
    char nodeName[14];
    DEBUG_PRINTF(nodeName, sizeof(nodeName), "TreeTap_%d", TREETAP_ID);
...
void setup() {
    ...
    state[8] = 0 ;
    state[9] = 0 ;
    DEBUG_PRINTF("TreeTap Node: %u initialized", TREETAP_ID);
    ...
    if (packetSize) {
        ...
        if (LoRaData[0] > 0){ //ttl
            ...
            if(TREETAP_ID==2){
                ...
                int count = (int32_t)(LoRaData[5]) | 
                    (int32_t)(LoRaData[4]) << 8 | 
                    (int32_t)(LoRaData[3]) << 16 | 
                    (int32_t)(LoRaData[2]) << 24;
